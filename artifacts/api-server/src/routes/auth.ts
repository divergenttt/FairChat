...
import { isValidPublicKey, normalizePublicKey } from "../lib/publicKey.js";
...
/** Random 200–500 ms delay to reduce timing-based account enumeration. */
function antiTimingDelay(): Promise<void> {
  const ms = 200 + Math.floor(Math.random() * 301);
...
router.post("/register", registerLimiter, async (req: Request, res: Response) => {
  ...
  const storedPublicKey = normalizePublicKey(publicKey);
  ...
  try {
    await db.execute(sql`
      ...
      VALUES (${userId}, ${cleanUsername}, ${trimmedDisplay}, ${passwordHash}, ${storedPublicKey}, ${storedRecoveryHash}, ${now})
    `);
  } catch (err: unknown) {
...
router.post("/restore/find", loginLimiter, async (req: Request, res: Response) => {
  ...
  if (recoveryHash) {
    ...
    await db.execute(sql`SELECT id FROM users WHERE recovery_hash = ${storedHash}`);
  } else {
    await db.execute(sql`SELECT id FROM users WHERE public_key = ${publicKey!} AND recovery_hash IS NULL`);
  }

  await antiTimingDelay();
  return res.json({ message: RESTORE_NEUTRAL_MESSAGE });
});
...
router.post("/restore/reset", loginLimiter, async (req: Request, res: Response) => {
  ...
  await antiTimingDelay();
  return res.json({
    message: RESTORE_NEUTRAL_MESSAGE,
    sessionExpiry: decodeExpiry(token)?.toISOString(),
    user: {
