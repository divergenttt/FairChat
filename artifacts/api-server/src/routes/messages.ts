...
router.post("/:recipientId", async (req: Request, res: Response) => {
  ...
  // Prevent database flooding: cap encryptedContent at 64 KB for text messages
  // and 4 KB for payment messages (which are small JSON, not E2E ciphertext).
  const MAX_CONTENT_TEXT = 65_536;
  const MAX_CONTENT_PAYMENT = 4_096;
  if (encryptedContent) {
    const maxLen = (msgType === "payment" || msgType === "payment_request" || msgType === "payment_request_declined")
      ? MAX_CONTENT_PAYMENT
      : MAX_CONTENT_TEXT;
    if (encryptedContent.length > maxLen) {
      return res.status(400).json({ error: `Content too long (max ${maxLen} characters)` });
    }
  }

  // For payment messages, validate the JSON payload structure so it's not easy
  // to forge a convincing fake payment card with arbitrary display values.
  if (msgType === "payment" && encryptedContent) {
    let parsed: Record<string, unknown>;
    try { parsed = JSON.parse(encryptedContent); } catch {
      return res.status(400).json({ error: "payment content must be valid JSON" });
    }
    const { amount, token, txHash } = parsed as { amount?: unknown; token?: unknown; txHash?: unknown };
    if (!amount || !token) {
      return res.status(400).json({ error: "payment content missing required fields" });
    }
    // txHash must be a valid Ethereum transaction hash if provided
    if (txHash !== undefined && txHash !== null && !/^0x[0-9a-fA-F]{64}$/.test(String(txHash))) {
      return res.status(400).json({ error: "payment txHash is not a valid Ethereum transaction hash" });
    }
  }
