                ...
                <div id={`msg-${msg.id}`}
                  ...
                  <div style={{ display:"flex", flexDirection:mine?"row-reverse":"row", alignItems:"flex-end", gap:6 }}>
                    ...
                    {msg.messageType === "payment" && (() => {
                      let pay: { amount?: string; token?: string; network?: string; txHash?: string; explorerUrl?: string | null; memo?: string } = {};
                      try { pay = JSON.parse(msg.decrypted ?? msg.encryptedContent); } catch {}
                    ...
                    {msg.messageType === "payment_request" && (() => {
                      let pay: { amount?: string; token?: string; network?: string; memo?: string } = {};
                      try { pay = JSON.parse(msg.decrypted ?? msg.encryptedContent); } catch {}
                                ...
                                <button
                                  onClick={e => {
                                    ...
                                    fetch(apiUrl(`/api/messages/${selectedUser.id}`), {
                                      ...
                                      credentials: "include",
                                      body: JSON.stringify({
                                        encryptedContent: JSON.stringify({ requestId: msg.id }),
