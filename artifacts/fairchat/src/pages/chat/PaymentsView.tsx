...
import { encryptMessage, getCachedPrivateKey } from "../../lib/crypto";
...
export default function PaymentsView() {
  ...
  useEffect(() => {
    ...
    fetch(apiUrl("/api/messages/payment-history"), { credentials: "include" })
      ...
      .then((data: TxHistoryEntry[]) => {
        setTxHistory(data.map(e => {
          let parsed: NonNullable<TxHistoryEntry["parsed"]> = {};
          try {
            parsed = JSON.parse(e.encryptedContent);
            if (parsed.networkId === "arc-confidential") {
              parsed.networkId = "arc";
            }
          } catch {}
          return { ...e, parsed };
        }));
  ...
  useEffect(() => {
    ...
    const payload = JSON.stringify({ amount, token: token.symbol, network: displayNetwork.name, networkId: outNetworkId, txHash, explorerUrl: network.explorerUrl ?? null, mode: paymentMode, ...(memo.trim() ? { memo: memo.trim() } : {}) });
    const myPrivKey = getCachedPrivateKey();
    const recipientPubKey = recipient.publicKey;
    if (!myPrivKey || !recipientPubKey) {
      console.warn("[PaymentsView] Missing keys — payment receipt not sent (encryption required)");
      return;
    }
    let encrypted: string;
    try {
      encrypted = encryptMessage(payload, recipientPubKey, myPrivKey);
    } catch (e) {
      console.error("[PaymentsView] Encryption failed — payment receipt not sent", e);
      return;
    }
  ...
  const handleRequest = async () => {
    ...
    const outNetworkId = effectiveNetworkId === "arc-confidential" ? "arc" : effectiveNetworkId;
    const payload = JSON.stringify({ amount, token: token.symbol, network: displayNetwork.name, networkId: outNetworkId, ...(memo.trim() ? { memo: memo.trim() } : {}) });
    try {
      const res = await fetch(apiUrl(`/api/messages/${recipient.id}`), { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ encryptedContent: payload, messageType: "payment_request" }) });
