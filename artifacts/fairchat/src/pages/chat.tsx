...
function fmtMsgPreview(decrypted: string | undefined, msgType?: string | null): string | undefined {
  ...
  if (msgType === "payment") {
    const p = tryParsePaymentPayload(decrypted);
    if (p) {
    ...
    const attemptSend = async (attempt: number) => {
      try {
        ...
        if (res.ok) {
          ...
          if (res.status === 403 && data.code === "RECIPIENT_NOT_IN_CONTACTS") {
            const markContactRequired = (prev: Message[]) =>
              prev.map(m => m.id === pendingId ? { ...m, _pending: false, _failed: true, deliveryStatus: "failed" as const } : m);
            if (target === selectedUser?.id) setMessages(markContactRequired);
            msgCacheRef.current.set(target, (msgCacheRef.current.get(target) ?? []).map(m =>
              m.id === pendingId ? { ...m, _pending: false, _failed: true, deliveryStatus: "failed" as const } : m));
            return;
          }
