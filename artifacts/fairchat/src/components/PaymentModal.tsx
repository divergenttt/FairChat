...
export default function PaymentModal({
  ...
  useEffect(() => {
    ...
    fetch(apiUrl("/api/messages/payment-history"), { credentials: "include" })
      ...
      .then((data: TxHistoryEntry[]) => {
        setTxHistory(
          data.map((e) => {
            let parsed: TxHistoryEntry["parsed"] = {};
  ...
  useEffect(() => {
    ...
    fetch(apiUrl(`/api/messages/${recipient.id}`), {
      ...
      credentials: "include",
      body: JSON.stringify({
        encryptedContent: payload, messageType: "payment",
        ...(initialReplyToRequestId ? { replyToId: initialReplyToRequestId } : {}),
      }),
    }).catch(() => {});
  }, [step, txHash, recipient?.id, amount, token.symbol, displayNetwork.name, effectiveNetworkId, network.explorerUrl, memo, initialReplyToRequestId, paymentMode]);
  ...
  const handleRequest = async () => {
    ...
    try {
      const res = await fetch(apiUrl(`/api/messages/${recipient.id}`), {
        ...
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ encryptedContent: payload, messageType: "payment_request" }),
      });
